...
\begin{figure*}[!t]
	...
	\end{subfigure}
	\hspace*{\fill}
	\caption[] {\label{fig:experimental_results} \textbf{Skill similarity and knowledge.} (\subref{fig:skill_similarity}) Similar skills in $\mathcal{S}$ can be grouped into clusters $\mathcal{Z}_k$, (\subref{fig:knowledge_idealization}) \myhl{remaining knowledge to learn a new skill $s_{j,k}$}\pending{Remove minus sign from figure}.}	
\end{figure*}
% ---
% ===================================================================================================
...
%	\begin{cases}
%		\left[  h_{j,k}\left(N_{\zeta_k},\alpha,\eta,\beta_k,r\right)  - \beta_k \sum_{l \in \mathcal{N}(j)}\gamma_{j,l}d(\bar{\sigma}_j,\bar{\sigma}_l)\right] \bar{\sigma}^{(\text{CL})}_{j,k}, & \epsilon < \bar{\sigma}_{j,k} < 1, \\
%		0, & \text{otherwise};
%	\end{cases}
%\end{equation}
\begin{equation}\label{eq:collective_knowledge_dynamics}
