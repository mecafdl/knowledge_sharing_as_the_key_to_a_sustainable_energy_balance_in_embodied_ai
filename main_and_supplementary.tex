...
%\begin{equation}\label{eq:collective_knowledge_dynamics}
%	\dot{\bar{\sigma}}^{(\text{CL})}_{j,k} =
%	\left[-f(\kappa;\alpha,\eta,\xi_k)  - h(\bar{\bm{\sigma}};\xi,\gamma_{j,l},m,)\right] \bar{\sigma}^{(\text{CL})}_{j,k}
%\end{equation}

\begin{equation}\label{eq:collective_knowledge_dynamics}
\dot{\bar{\sigma}}^{(\text{CL})}_{j} =
-\left[-\underbrace{f(\kappa;\alpha,\eta,\bm{B},N_r)}_{\text{self-learning dynamics}} + \overbrace{h(\bar{\bm{\sigma}};\bm{B},\bm{\Gamma},N_r) \bar{\sigma}^{(\text{CL})}_{j}}^{\text{knowledge sharing dynamics}}\right]
\end{equation}
