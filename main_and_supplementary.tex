...
% The expression shown in Fig.~\ref{fig:collective_learning_and_skill_manifold_conceptualization}~\textbf{B}---and detailed in Eq.~\eqref{eq:collective_knowledge_dynamics} in \nameref{sec:methods}---describes the dynamics of the remaining knowledge $\dot{\bar{\sigma}}^{(\mathrm{CL})}_{i,j}$ about a skill~$j$ learned by an \ac{eai} agent~$i$ within a \ac{cl} system. \myhl{This equation depends on four key terms that govern the behavior of the \ac{cl} system: \textbf{Agent Learning Gain} ($\alpha_i$), which captures the inherent ability of agent~$i$ to acquire knowledge about skill~$j$, with the learning gains for all agents collected in the vector~$\bm{\alpha}$; \textbf{Intra-Cluster Knowledge Sharing Gain} ($\eta_i$), which quantifies how effectively agent~$i$ can reinforce skill~$j$ through related skills in its memory that belong to the same cluster, with the vector~$\bm{\eta}$ aggregating these gains across agents; \myhl{\textbf{Inter-Cluster Similarity Matrix} ($\bm{B} \in \mathbb{R}^{N_\mathcal{K} \times N_\mathcal{K}}$), which weighs the contribution of skills from other clusters to skill~$j$ based on structural similarity within the skill manifold. For each agent~$i$ learning skill~$j$, a corresponding vector~$\bm{\beta}_{ij}$ is derived from the entries of~$\bm{B}$ and the skills present in the agent’s memory}; and \textbf{Inter-Agent Transfer Gains} ($\bm{\Gamma} \in \mathbb{R}^{N_\mathrm{r} \times N_\mathrm{r}}$), where each entry~$\gamma$ models the transfer of knowledge between pairs of agents, allowing for concurrent learning across the $N_\mathrm{r}$ agents in the system through skill-specific interactions.}
...
% To analyze how these two factors shape system-level dynamics, we studied an idealized---the particulars of this scenario are provided in the \nameref{sec:supplementary_materials}---where each of the $N_\mathrm{r}$ \ac{eai} agents in the collective learns a different skill in every learning cycle.\footnote{A learning cycle encompasses all episodes required to learn a given skill batch.} Based on all possible combinations of $\bar{\eta}$ and $\bar{\gamma}$, we identified nine canonical \ac{cl} regimes---summarized in Table~\ref{tab:cl_regimes}---that can also be categorized into four principal regimes. Fig.~\ref{fig:collective_learning_and_skill_manifold_conceptualization}~\textbf{C} illustrates three of these principal regimes, each depicted through a schematic of agent-skill-manifold interaction, Gaussian distributions over $\eta$ and $\gamma$, and performance plots indicating the number of learning episodes required to acquire all $N_\mathcal{S}$ skills as a function of collective size. The energy expenditure of the learning process is related to the learning episodes. For simplicity, the energetic cost of a learning episode is assumed to be constant and is represented by $ e_0 $. Consequently, the total energy expenditure to learn all skills is directly proportional to the total number of learning episodes. These performance plots also show the success rate, defined as the percentage of skills successfully learned.
