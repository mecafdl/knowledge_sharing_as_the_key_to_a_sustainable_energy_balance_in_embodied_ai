...
%%
clc
c_jk_cl_dist_episodes_ParamSweep = [];
learnedSkillsStorage = [];
for i=1:5
    tmp = load(['newResultsMoreRobots_' num2str(i)],'c_jk_cl_dist_episodes_ParamSweep');
    c_jk_cl_dist_episodes_ParamSweep = [c_jk_cl_dist_episodes_ParamSweep , tmp.c_jk_cl_dist_episodes_ParamSweep ];
    tmp = load(['newResultsMoreRobots_' num2str(i)],'learnedSkillsStorage');
    learnedSkillsStorage = [learnedSkillsStorage , tmp.learnedSkillsStorage ];
end

tmp = zeros(32,50,numel(c_jk_cl_dist_episodes_ParamSweep));
for i=1:numel(c_jk_cl_dist_episodes_ParamSweep)
tmp(:,:,i) = c_jk_cl_dist_episodes_ParamSweep{i};
end
[~,c_jk_cl_dist_episodes_ParamSweep,c_jk_cl_dist_episodes_ParamSweep_std] = zscore(tmp,0,3);
c_jk_cl_dist_episodes_ParamSweep = {c_jk_cl_dist_episodes_ParamSweep};
...
x = 4:4:parameters.maxNumberOfRobots;
...
colormap hot
% yticklabels({'4','8','16',x'32','64','128'})
yticks([1:32])
yticklabels(arrayfun(@(i) num2str(robotArray(i)),1:numel(robotArray),'UniformOutput',false))
aux =round(linspace(-0.2,1,size(complexities{1},2)),2);
...
if SAVE_FIG == 1
    exportgraphics(gcf, fullfile(fileparts(matlab.desktop.editor.getActiveFilename), ...
        'figures','learned_skills_percentage_v2.png'),'Resolution',600)
    close(gcf);
end 
...
robotArray = (4:4:parameters.maxNumberOfRobots);
...
upperBound  =  12800*ones(numel(robotArray),1);
% lowerBound= parameters.totalSkills./[4,8,16,32,64,128]'.*parameters.fundamentalComplexity;
lowerBound= floor(parameters.totalSkills./robotArray').*parameters.fundamentalComplexity;
x = 1:numel(robotArray);
% upperBound = parameters.totalSkills./[4,8,16,32,64,128]'.*parameters.fundamentalComplexity;
% lowerBound = upperBound - 300;
patch([x fliplr(x)], [lowerBound'  fliplr(upperBound')], [0.5  0.5  0.5],'FaceAlpha',0.25,'EdgeColor','w');
...
% plot(5*ones(size(1E1:100:1E5)),1E0:100:1E5,'k--','LineWidth',3)
xticks([1:numel(robotArray)])
xticklabels(arrayfun(@(i) num2str(robotArray(i)),1:numel(robotArray),'UniformOutput',false))
xlabel('Number of robots','FontSize',25)
ylabel('Complexity (episodes for all skills)','FontSize',25)
...
sweetPoints = cell2mat(arrayfun(@(i) find(learnedSkillsRate(i,:)==100,1,'first'),1:32,'UniformOutput',false));
...
for i=1:numel(sweetPoints)
    if ~isnan(sweetPoints(i))
        ...
        text(bla(theLine).XData(i), ...
                bla(theLine).YData(i),num2str(aux(theLine)))   
    end
end
...
if SAVE_FIG == 1
    exportgraphics(gcf, fullfile(fileparts(matlab.desktop.editor.getActiveFilename), ...
        'figures','total_episodes_for_all_skills_eta_n0.1_v3.png'),'Resolution',600)
    close(gcf);
end  
...
function [c_jk_cl_dist_episodes, learnedSkillsStorage] = runCollectiveLearningDistributed(parameters)
    ...
    totalGammaCandidates      = 50;
    theGammas                 = linspace(-0.2,1,totalGammaCandidates);
    learnedSkillsStorage      = zeros(32,numel(theGammas));
    ...
    for gammaScenario = 1:totalGammaCandidates
        ...
        for numberOfRobots = 4:4:parameters.maxNumberOfRobots
            fig = figure('color','w');
            ...
            pause(1)
            tightfig(fig);
            c_jk_cl_dist_episodes(robotBatchIndex, gammaScenario) = sum(c_jk_cl_dist);
            learnedSkillsStorage(robotBatchIndex, gammaScenario) = learnedSkills;
