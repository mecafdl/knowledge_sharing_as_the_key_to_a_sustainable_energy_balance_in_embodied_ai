...
% Knowledge --------------------------------------------------------------
figure('Color',[1 1 1])
plot(1:N_S/N_K, sigma_bar,'-or','LineWidth', 2)
xlabel('$N_{j}$','Interpreter','latex','FontSize',20);
...
area(j) = trapz(n, sigma_bar(j)*exp(-6/cj(j)*n));
...
for k = 1:N_K
    ...
    transfer(k) = 1 - beta*sum(sigma);
    plot(k*c0*ones(11,1),0:0.1:1,'k-','LineWidth', 0.5)
    for j = 1:N_S/N_K
        sigma_bar(j,k) = transfer(k)*exp(-alpha*(j-1));
        cjk(j,k)       = c0*sigma_bar(j,k);
        n              = 0:0.1:cjk(j,k);
        p              = plot(n + (k-1)*c0,sigma_bar(j,k)*exp(-6/cjk(j,k)*n),'LineWidth',1);
        p              = area(n + (k-1)*c0,sigma_bar(j,k)*exp(-6/cjk(j,k)*n),'EdgeColor',p.Color,'FaceColor',p.Color,'FaceAlpha',0.1);
        C              = C + cjk(j,k);
    end
    ax = gca;
