...
\end{equation}
% ---
Notice that if Eq.~\eqref{eq:skill_energy_in_eai} was used to describe the energy consumption of a task in CAI, $E_{BEE}$ could be associated with the edge devices and $E_{CCE}$ would represent the main source of energy consumption. Crucially, in CAI the expenditures $E^\star_{\tau}$ and $E_{MIE}$ do not exist since physical interaction is absent.

\section*{Related works}
\redtext{The trends depicted in Fig.~\ref{fig:energy_demands_AI_robotics} indicate that the energy expenditures for computation and communication, basal functions, and motion and interaction will likely follow a similar pattern. The implication is straightforward: as the number of AI applications increases, so does the demand for energy. Only recently have the energy requirements of both classical and embodied AI received significant attention within the AI and robotics research communities.\\
...
This paradigm goes beyond simple parallelization. In CL $m$ robotic agents $ \left\lbrace r_i \right\rbrace_{i=1}^{m} $ develop and accumulate a common mind (body of knowledge) dynamically via networked interactions where individual experience, knowledge, and skills are disseminated to all the other elements in the collective \cite{Garavan2012CollectiveLearning}. Information flows vertically as previous knowledge is passed on and horizontally by sharing concurrent experience between agents. Knowledge can be replicated, complemented, and further developed via these mechanisms. Moreover, to enable CL, it is assumed that an inter-agent communication protocol and the appropriate infrastructure are in place that enables agents to concurrently exchange and integrate the self-acquired and received knowledge to incrementally speed up the learning of all the agents as a whole. As a result, concurrent intra- and inter-cluster knowledge share is possible. Naturally, the CL paradigm involves a complex scheduling problem to determine the optimal skill distribution and inter-agent knowledge-sharing strategy. 
