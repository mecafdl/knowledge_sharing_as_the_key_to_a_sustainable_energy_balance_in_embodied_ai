...
% figure
% N_array = 0:N_Z;
% % cj = -log(threshold)./((N_array + 1)*alpha);
% cj = 1./((N_array + 1)*alpha);
% plot(N_array, cj)
% hold on;stem(N_array,kappa)
% % C_tot = sum(cj)
...
% theta          = normrnd(5*ones(m*(m+1)/2,1),0.33*ones(m*(m+1)/2,1));
theta          = 1*ones(m*(m+1)/2,1);
% theta(N+1:end) = rand(N*(N+1)/2 - N,1);

% theta(m+1:end) = rand(m*(m+1)/2 - m,1);
gamma          = 0.7;%rand(1);
...
%         gamma = 0.07;%
        gamma = 0.1*(abs(a)/max(eig(abs(W))));
        test = [a, gamma,abs(a)/max(eig(abs(W))), gamma < abs(a)/max(eig(abs(W)))]
        F = a*eye(m) + gamma*W; 
        eig(F)
        %bsigma   = transpose(ode4(@(n,bsigma) A*r*f(eta, N_zeta)*bsigma, n, bsigma_0));
