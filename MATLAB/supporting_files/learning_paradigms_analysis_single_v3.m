...
sigbar = @(Nj) exp(-alpha*Nj);
rate = 6;
knowledge = @(n,cj) exp(-rate/cj*n);
...
for j = 1:N_S/N_K % loop over the skills in a cluster
    ...
    cj(j)        = c0*sigma_bar(j);
    n            = 0:0.001:cj(j); 
    p = plot(n + 1*C,  (sigma_bar(j) - sigbar(j))*knowledge(n,cj(j)) + 1*sigbar(j));
%     p = plot(n + 1*C,  (sigma_bar(j) - sigbar(j))*(exp(-6/cj(j)*n)) + 1*sigbar(j));
    
%     p = area(n + 1*C,  (sigma_bar(j) - sigbar(j))*(exp(-6/cj(j)*n)) + 1*sigbar(j),'FaceAlpha',0.5);
    hold on
    p = area(n + 1*C,  (sigma_bar(j) - sigbar(j))*knowledge(n,cj(j)) + 1*sigbar(j),'FaceAlpha',0.5);
    C = C + cj(j);
end
sigma_bar(j+1) = sigbar(N_S/N_K);
...
% title('Remaining Knowledge VS. Episode (Single Incremental)sigbar(j-1);','FontSize',15)
grid on
% Knowledge --------------------------------------------------------------
figure('Color',[1 1 1])
plot(0:N_S/N_K, sigma_bar,'-or','LineWidth', 2)
xlabel('$N_{j,k}$','Interpreter','latex','FontSize',20);
ylabel('${\bar{\sigma}}_{j,k}^{(I)}$','Interpreter','latex','FontSize',20); 
% xlim([1 N_S/N_K])
...
%             p = plot(n + shift*sum(cjk(1:j-1,k)),  (bar_sigma_jk(j,k) - transfer(k)*sigbar(j))*knowledge(n,cjk(j,k)) + transfer(k)*sigbar(j));
...
%             p = plot(n + shift*sum(cjk(1:j-1,k)),  (bar_sigma_jk(j,k) - transfer(k)*sigbar(j))*knowledge(n,cjk(j,k)) + transfer(k)*sigbar(j));
    ...
    sigma_k(k)         = 1 - transfer(k)*sigbar(N_S/N_K);
    crossing = 1-sigma_bar_S;
    tmp = [tmp, dsigma_bar_S(k)*1*sigbar(0:N_S/N_K)+ crossing(k+1)];
    plot([0;cumsum(cjk(:,k))]+sum(cjk(:,1:k-1),'all') ,dsigma_bar_S(k)*transfer(k)*sigbar(0:N_S/N_K)/transfer(k)+...
       crossing(k+1),'r:o','MarkerSize',5,...
    'MarkerEdgeColor','red',...
    'MarkerFaceColor',[1 .6 .6])
%     plot([0;cumsum(cjk(:,k))]+sum(cjk(:,1:k-1),'all') ,dsigma_bar_S(k)*transfer(k)*sigbar(0:N_S/N_K)+...
%        1*(transfer(k) - dsigma_bar_S(k)*transfer(k)*sigbar(0)),'r:o','MarkerSize',5,...
%     'MarkerEdgeColor','red',...
%     'MarkerFaceColor',[1 .6 .6])    
end
...
ylabel('$\bar{\sigma}^{(T)}_{\mathcal{S}}$','Interpreter','latex','FontSize',25);
xlim([0 N_K])
ylim([0 1])
% title('Complexity VS. Skills (Single Transfer)','FontSize',15)
grid on
bar_sigma_jk=[bar_sigma_jk;zeros(1,10)];
% -------------------------------------------------------------------------
figure('Color',[1 1 1])
for k = 1:N_K
    plot((0:N_S/N_K) + (k-1)*N_S/N_K, dsigma_bar_S(k)*bar_sigma_jk(:,k)/transfer(k) + crossing(k+1),'or-','LineWidth',2,...
    'MarkerSize',2,...
    'MarkerEdgeColor','red',...
    'MarkerFaceColor',[1 .6 .6])
    hold on
    plot(k*10*ones(11,1),0:0.1:1,'k-','LineWidth', 0.5)
end
plot((0:N_K)*N_S/N_K,1 - sigma_bar_S ,'ko-','LineWidth',2)
xlabel('$s_{j,k}$','Interpreter','latex','FontSize',25);
ylabel('$\bar{\sigma}^{(T)}_{\mathcal{S}}$','Interpreter','latex','FontSize',25);
xlim([0 N_S])
ylim([0 1])
% title('Complexity VS. Skills (Single Transfer)','FontSize',15)
