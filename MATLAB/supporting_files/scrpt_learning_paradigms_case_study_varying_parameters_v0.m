...
for scenario = 1:parameters.totalSimulationScenarios
    ...
    %     legend(p_aux,['m = ', num2str(m)]);
        ...
        c_jk_iso_parameters.episodes(robotBatchIndex,:,scenario) = [sum(c_jk_iso,1), sum(c_jk_iso,'all')] ;
...
for scenario = 1:parameters.totalSimulationScenarios
    ...
    for numberOfRobots = [2,4,8,16,32,64,128]
        ...
        c_jk_il_parameters.episodes(robotBatchIndex,:,scenario) = [sum(c_jk_il,1), sum(c_jk_il,'all')];
        robotBatchIndex = robotBatchIndex + 1;
    end
end
%%
...
for scenario = 1:parameters.totalSimulationScenarios
    ...
    for numberOfRobots = [2,4,8,16,32,64,128]
        ...
        c_jk_til_parameters.episodes(robotBatchIndex,:,scenario) = [sum(c_jk_itl,1), sum(c_jk_itl,'all')] ;
        robotBatchIndex                      = robotBatchIndex + 1; 
    end
end
%%
...
for scenario = 1:parameters.totalSimulationScenarios
    ...
    for numberOfRobots = [2,4,8,16,32,64,128]   
        ...
        % Coupling
        % gamma_0 = 0.05;
        %Gamma   = 0.25*parameters.eta_0.*randn(numberOfRobots,numberOfRobots) + gamma_0;
        gamma_0 = 0.1;
        Gamma   = 0 + (gamma_0-0).*rand(numberOfRobots,numberOfRobots);
        ...
        for skillCluster = 1:parameters.totalSkillClusters
            ...
            for learnedSkills = 0:parameters.skillsPerCluster/numberOfRobots-1    
                ...
                eta_robots = max((parameters.eta_std*parameters.eta_0.*randn(numberOfRobots,1) + parameters.eta_0),0);
                selfDynamicsMatrix          = -Alpha.*((1 - beta_k).^(-1)).*f(eta_robots, r*learnedSkills).*eye(numberOfRobots);
                % F        = a*eye(m) + gamma.*A;
                % lambda_F = eig(F);
                % disp(lambda_F)
                ...
                %     return
                % end
                %sigmaBar   = transpose(ode4(@(n,sigmaBar) F*sigmaBar, n, initialRemainingKnowedge));
                % remainingKnowledge   = transpose(ode4(@(n,sigmaBar) fcn_knowledge_integration(sigmaBar, numberOfRobots, diag(a), gamma_0.*A), parameters.episodes, initialRemainingKnowedge));
                remainingKnowledge   = transpose( ...
                    ode4(@(n,sigmaBar) fcn_collective_knowledge_dynamics(sigmaBar, numberOfRobots, selfDynamicsMatrix, Gamma), parameters.episodes, initialRemainingKnowedge));
                %c_jk_cl(j, k)  = parameters.episodes(min(find(remainingKnowledge(1,:)<epsilon)));
                ...
                %semilogy(parameters.episodes,remainingKnowledge,'LineWidth',2)
                % semilogy(parameters.episodes,remainingKnowledge,'k-','LineWidth',1)
        ...
        c_jk_cl_parameters.episodes(robotBatchIndex,:,scenario) = [sum(c_jk_cl,1), sum(c_jk_cl,'all')] ;
        robotBatchIndex                     = robotBatchIndex + 1; 
    end
end
simTime = toc;
    ...
    for numberOfRobots = [2,4,8,16,32,64,128]   
        ...
        for skillCluster = 1:parameters.totalSkillClusters
            ...
            for skillsBatch = 1:parameters.skillsPerCluster/numberOfRobots
                ...
                Alpha   = diag(parameters.alpha_min + (parameters.alpha_max-parameters.alpha_min).*rand(numberOfRobots,1));
                % New set of skills -> new inter agent coupling
                %gamma_0 = 0.05;
                Gamma   = fcn_generateGamma(parameters.gamma_0, parameters.gamma_std, numberOfRobots);
                % r       = double(gamma_0==0) + double(gamma_0~=0)*numberOfRobots;
                ...
                % Sample eta factor from Gaussian distribution
                % eta_robots = parameters.eta_0 + parameters.eta_std*parameters.eta_0.*randn(numberOfRobots,1);
                % F = pearspdf(X, parameters.eta_0, parameters.eta_std*parameters.eta_0, -4, 100);
                % eta_robots   = pearsrnd(parameters.eta_0, parameters.eta_std*parameters.eta_0, 2, 100, numberOfRobots,1);
                % eta_robots   = pearsrnd(parameters.eta_0, parameters.eta_std*parameters.eta_0, -4, 100, numberOfRobots,1);
                eta_robots   = pearsrnd(parameters.eta_0, parameters.eta_std, 1, 10, numberOfRobots, 1);
                               % pearsrnd(parameters.eta_0, parameters.eta_std, 1, 10,5,1)

                % Set self-learning dynamics
                ...
                %         Gamma, ...
                %         true, ...
                %         parameters.knowledgeLowerBound), parameters.episodes, initialRemainingKnowedge));
...
for scenario = 1:parameters.totalSimulationScenarios
    ...
    for numberOfRobots = [4,8,16,32,64,128]
        ...
        % beta_k = (1/parameters.totalSkillClusters);
        % B      = beta_k*A;
        % for i=1:parameters.totalSkillClusters
        %    B((numberOfRobots/parameters.totalSkillClusters)*(i-1) + 1:(numberOfRobots/parameters.totalSkillClusters)*i,(numberOfRobots/parameters.totalSkillClusters)*(i-1) + 1:(numberOfRobots/parameters.totalSkillClusters)*i) = ones(numberOfRobots/parameters.totalSkillClusters) - eye(numberOfRobots/parameters.totalSkillClusters);
        % end
    ...
    %     %     subplot(2,N_K,k)        
    %         N_zeta   = k-1;
    %         initialRemainingKnowedge = (1- beta_k)*g(parameters.delta, r*N_zeta).*ones(m,1);
    ...
    %     end
        for learnedSkills = 0:(parameters.totalSkills/numberOfRobots)-1
            ...
            % ============================================
            beta_k = r*learnedSkills/parameters.totalSkills;%.*rand(numberOfRobots,1); % From each cluster, only this fraction of knowlede can be transfered
    %         beta_k = (r*N_zeta/N_Z)*(1/N_K)%min(r*N_zeta/N_Z,1)*(1/N_K)
            ...
            % perturbationB = 0.95 + (1.0-0.95).*rand(numberOfRobots,numberOfRobots); %rand(numberOfRobots,numberOfRobots);
            perturbationB(B==0) = 1;
            perturbationB(B==1) = 1;
            ...
            initialRemainingKnowedge = (1 - beta_k)*g(parameters.delta, r*learnedSkills).*ones(numberOfRobots,1);
            eta_robots = abs(0.1*parameters.eta_0.*randn(numberOfRobots,1) + parameters.eta_0);
            selfDynamicsMatrix          = -Alpha.*f(eta_robots, r*learnedSkills).*eye(numberOfRobots);        
            %sigmaBar   = transpose(ode4(@(n,sigmaBar) F*sigmaBar, n, initialRemainingKnowedge));
            % remainingKnowledge   = transpose(ode4(@(n,sigmaBar) fcn_collective_knowledge_dynamics(sigmaBar, numberOfRobots, selfDynamicsMatrix, Gamma.*B), parameters.episodes, initialRemainingKnowedge));
            ...
            % c_jk_cl_dist(j) = parameters.episodes(min(find(remainingKnowledge(1,:)<epsilon)));
            % loglog(parameters.episodes,remainingKnowledge,'k-',LineWidth=1)
...
leg = legend(p,'CL$_1$','CL$_2$','CL$_\mathrm{distributed}$');
