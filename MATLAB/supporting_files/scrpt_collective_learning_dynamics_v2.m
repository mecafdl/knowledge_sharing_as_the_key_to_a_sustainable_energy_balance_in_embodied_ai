...
% A_i       = -diag(tau/c0);
alpha     = -log(threshold)/c0;
A_i       = -alpha*eye(3);

cond = false;
while ~cond
    %A_nd = triu(rand(3),1);
    beta = -0.01;%alpha*rand(1);
    A_nd = beta*triu(ones(3),1);
    A_nd =  A_nd + transpose(A_nd);
%     A    = A_nd - rand(1)*eye(3);
    A    = A_nd + A_i;
    cond = all(eig(A)<0);
end
...
n = 0:0.01:c0;
% x = ode4(@(t,x) -0.1*L*x + 0*([1;0;0] - exp(-0.1*t)), n, rand(N,1));
% x = ode4(@(t,x) -0.1*L*x, n, [1;0.9;0.6]);
x = ode4(@(t,x) A*x, n, [1; 1; 1]);
close all
fig = figure('Color','w');
fig.WindowState = 'maximized';
plot(n,x)
hold on
plot(n,vecnorm(x'),'k','LineWidth',2)
% figure
% plot(n,1-exp(-0.1*n))
...
cond = false;
while ~cond
    %A = A_nd - diag(rand(3,1));
    A = A_nd - rand(1)*eye(3);
    %cond = all(eig(A)<0);
    cond = all(abs(eig(A))<1);
end
...
theta          = 1*ones(N*(N+1)/2,1);
% theta(N+1:end) = rand(N*(N+1)/2 - N,1);
beta           = 1;%rand(1);
theta(N+1:end) = beta*ones(N*(N+1)/2 - N,1);
ind            = find(triu(ones(N,N),1));
L              = zeros(N) + diag(theta(1:N));
L(ind)         = theta(N+1:end);
G              = L + triu(L,1)';
eig(G);
% G = (L')*L
...
x = ode4(@(t,x) ...
      fcn_general_dynamics(t, x, A, alpha, s, c0, tau, threshold, learning_type_1), ...
                                     n, ...
                                     ones(N,1));
...
x_nonebounded = ode4(@(t,x) ...
      fcn_general_dynamics(t, x, A, alpha, s, c0, tau, threshold, learning_type_2), ...
                                     n, ...
                                     ones(N,1));
x_nonebounded = x_nonebounded';
% -------------------------------------------------------------------------
figure('Color','w')
...
p = area(n,  threshold*ones(numel(n),1),'FaceColor',[0 1 0],'FaceAlpha',0.25,'EdgeColor','w');
plot(n,threshold*ones(numel(n),1),'k:','LineWidth',2);
