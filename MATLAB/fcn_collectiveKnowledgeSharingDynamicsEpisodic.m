function d_coupledRemainingKnowledge = ...
    ...
    selfKnowledgeDynamics  = -Alpha.*((1 - totalTrasferableKnowledgeFraction).^(-1)).*f(eta_robots, numberOfLearnedSkills).*eye(numberOfRobots);


selfKnowledgeDynamics  = -Alpha.*diag((1 - parameters.clusterTransferrableKnowledgeFractionPerAgent).^(-1)).*f(eta_robots, numberOfLearnedSkills).*eye(numberOfRobots);
...
clusterTransferMatrix = repmat(parameters.clusterTransferrableKnowledgeFractionPerAgent,numberOfRobots,1).*adjacencyMatrix;
